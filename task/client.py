...
class DialClient:
    ...
    def __init__(
        ...
        self._tools_list = [tool.schema for tool in tools] if tools else []
        print(f"DialClient initialized with endpoint: {self._endpoint}")
        print(f"Available tools: {json.dumps(self._tools_list, indent=2)}")


    def get_completion(self, messages: list[Message], print_request: bool = False) -> Message:
        #TODO:
